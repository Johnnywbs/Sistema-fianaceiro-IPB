...
if($total_reg == 0){
  $pdo->query("INSERT into config SET empresa = '$id_empresa', nome_sistema = 'Sistema de Vendas', tipo_rel = 'PDF', foto_rel = 'logo-rel.jpg', tipo_desconto = '%', comissao = '0' ");
	...
	echo $res[0]['nome_sistema'].'-*'.$res[0]['email_sistema'].'-*'.$res[0]['telefone_sistema'].'-*'.$res[0]['tipo_rel'].'-*'.$res[0]['foto_rel'].'-*'.$res[0]['tipo_desconto'].'-*'.$res[0]['comissao'].'-*'.$res[0]['endereco_sistema'].'-*'.$res[0]['cnpj_sistema'].'-*'.$res[0]['token'].'-*'.$res[0]['instancia'].'-*'.$res[0]['token_pix'].'-*'.$res[0]['chave_pix_sistema'];
}
