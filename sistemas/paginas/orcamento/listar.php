...
$base_icmsF = number_format($base_icms, 2, ',', '.');
$valor_icmsF = number_format($valor_icms, 2, ',', '.');
$valor_prodF = number_format($valor_prod, 2, ',', '.');
$valor_seguroF = number_format($valor_seguro, 2, ',', '.');
$valor_freteF = number_format($valor_frete, 2, ',', '.');
$descontoF = number_format($desconto, 2, ',', '.');
$valor_ipiF = number_format($valor_ipi, 2, ',', '.');
$valor_notaF = number_format($valor_nota, 2, ',', '.');
...
<td><i class="fa fa-square {$classe_pago} mr-1"></i></td>
				...
				<td>
					<big><a class="{$ocultar}" href="#" onclick="editar('{$id}', '{$numero_documento}', '{$serie_nf}','{$tipo_pagamento}','{$descricao_pgto}','{$data_emiss}','{$data_venc}','{$pessoa}','{$chave_nf}','{$cod_aut}','{$data_pgto}','{$valor}','{$base_icms}','{$valor_icms}','{$valor_prod}','{$valor_seguro}','{$valor_frete}','{$desconto}','{$valor_ipi}','{$valor_nota}','{$compensacao}','{$frequencia}','{$centro_de_custo}','{$sub_centro}','{$classificacao}','{$reembolso}','{$tumb_arquivo}', '{$tag}')" title="Editar Dados"><i class="fa fa-edit text-primary "></i></a></big>
					...
					<big><a href="#" onclick="mostrar('{$id}', '{$numero_documento}', '{$serie_nf}','{$tipo_pagamento}','{$descricao_pgto}','{$data_emissF}','{$data_vencF}','{$nome_format}','{$chave_nf}','{$cod_aut}','{$data_lancF}','{$data_pgtoF}','{$valorF}','{$base_icmsF}','{$valor_icmsF}','{$valor_prodF}','{$valor_seguroF}','{$valor_freteF}','{$descontoF}','{$valor_ipiF}','{$valor_notaF}','{$compensacao}','{$nome_frequencia}','{$centro_de_custo}','{$sub_centro}','{$classificacao}','{$reembolso}','{$tumb_arquivo}','{$nome_usu_lanc}','{$nome_usu_pgto}','{$pago}','{$arquivo}', '{$tag}') " title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function editar(id, numero_documento, serie_nf, tipo_pagamento,descricao_pgto, data_emiss, data_venc, pessoa, chave_nf,cod_aut, data_pgto, valor,base_icms, valor_icms,
					valor_prod, valor_seguro,valor_frete,desconto,valor_ipi,valor_nota,compensacao,frequencia,centro_de_custo,sub_centro,classificacao,reembolso,arquivo,tag){
		...
		$('#valor_prod').val(formatarReal(valor_prod));
	...
	function mostrar(id, numero_documento, serie_nf, tipo_pagamento, descricao_pgto, data_emiss, data_venc, pessoa, chave_nf,cod_aut, data_lanc, data_pgto, valor,base_icms, valor_icms,
					valor_prod, valor_seguro,valor_frete,desconto,valor_ipi,valor_nota,compensacao,frequencia,centro_de_custo,sub_centro,classificacao,reembolso,arquivo, usuario_lanc, usuario_pgto, pago, link,tag){
		...
		$('#base_icms_mostrar').text(base_icms);
		$('#valor_icms_mostrar').text(valor_icms);
		$('#valor_prod_mostrar').text(valor_prod);
		$('#valor_seguro_mostrar').text(valor_seguro);
		$('#valor_frete_mostrar').text(valor_frete);
		$('#desconto_mostrar').text(desconto);
		$('#valor_ipi_mostrar').text(valor_ipi);
		$('#valor_nota_mostrar').text(valor_nota);
