...
$query_con = $pdo->query("SELECT * FROM produtos WHERE codigo = '$codigo' and empresa = '$id_empresa'");
$res = $query_con->fetchAll(PDO::FETCH_ASSOC);
if(@count($res) > 0){
	...
	if($estoque > 0){
		...
		$res = $pdo->prepare("UPDATE produtos SET estoque = :estoque, vendas = '$nova_venda' WHERE id = '$id' and empresa = '$id_empresa'");
