...
<small>
	...
	<th class="esc">Valor Compra(u)</th>	
...
<td class="esc">R$ {$valor_compra}</td>
...
<td>
HTML;
if($nivel_usuario == 'Administrador' || $nivel_usuario == 'SAS'){
echo <<< HTML
...
<big><a href="#" onclick="mostrar('{$codigo}','{$nome}','{$descricao}','{$estoque}','{$valor_vendaF}','{$valor_compraF}','{$lucro}', '{$nome_forn}', '{$nome_cat}', '{$foto}', '{$data_cadF}', '{$ativo}','{$nivel_estoque}','{$vendas}')" title="Mostrar Dados"><i class="fa fa-info-circle text-primary"></i></a></big>
...
<big><a href="#" onclick="comprar('{$id}', '{$lucro}', '{$valor_venda}', '{$valor_compra}', '{$nome}', '{$total_grade}')" title="Comprar Produtos"><i class="fa fa-shopping-cart" style="color:#741b47"></i></a></big>

<big><a href="#" onclick="grade('{$id}', '{$nome}')" title="Grade Produtos"><i class="fa fa-list" style="color:#056704"></i></a></big>

<big><a href="#" onclick="entrada('{$id}', '{$nome}', '{$total_grade}')" title="Entrada Produtos"><i class="fa fa-sign-in" style="color:#1d6aeb"></i></a></big>

<big><a href="#" onclick="saida('{$id}', '{$nome}', '{$total_grade}')" title="Saída Produtos"><i class="fa fa-sign-out text-danger" ></i></a></big>
...
<script type="text/javascript">
	...
	function mostrar(codigo, nome, descricao, estoque, venda, compra, lucro, fornecedor, categoria, foto, data, ativo, nivel_estoque, vendas){
		...
		$('#vendas_produtos_dados').text(vendas);
	...
	function comprar(id, lucro, venda, compra, nome, grade){
		...
		if(grade == 0){
              $('#grade-comprar-1').css("display", "none");
              $('#grade-comprar-2').css("display", "none");
            }else if(grade == 1){
              $('#grade-comprar-2').css("display", "none");
              $('#grade-comprar-1').css("display", "flex");
            }else{
            	 $('#grade-comprar-2').css("display", "flex");
              $('#grade-comprar-1').css("display", "flex");
            }
	...
	function entrada(id, nome, grade){	
	...
	if(grade == 0){
              ...
              $('#grade-entrada-2').css("display", "none");
              $('#grade-entrada-1').css("display", "flex");
            }else{
            	 $('#grade-entrada-2').css("display", "flex");
              $('#grade-entrada-1').css("display", "flex");
            }	
	...
	function saida(id, nome, grade){		
		...
		if(grade == 0){
              ...
              $('#grade-saida-2').css("display", "none");
              $('#grade-saida-1').css("display", "flex");
            }else{
            	 $('#grade-saida-2').css("display", "flex");
              $('#grade-saida-1').css("display", "flex");
            }	
