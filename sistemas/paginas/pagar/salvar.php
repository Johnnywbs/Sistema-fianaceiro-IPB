...
$valor_bruto = formatacao($valor_bruto);
...
$pis_pasep = formatacao($pis_pasep);

$cofins = $_POST['cofins'];
$cofins = formatacao($cofins);

$csll = $_POST['csll'];
$csll = formatacao($csll);

$irrf = $_POST['irrf'];
$irrf = formatacao($irrf);
...
if($id == ""){
	$query = $pdo->prepare("INSERT INTO $tabela SET empresa = '$id_empresa', numero_documento = '$numero_documento', serie_nf = '$serie_nf',tipo_pagamento = '$tipo_pagamento',data_emiss = '$data_emiss', data_lanc = curDate(), descricao_pgto = '$descricao_pgto', fornecedor = '$fornecedor', chave_nf = '$chave_nf',cod_aut = '$cod_aut', data_venc = '$data_venc', data_pgto = '$data_pgto', valor = :valor, base_icms = :base_icms, valor_icms = :valor_icms,
													valor_bruto = :valor_bruto, valor_seguro = :valor_seguro, pis_pasep = :pis_pasep, cofins = :cofins, csll = :csll, irrf = :irrf,iss = :iss, valor_frete = :valor_frete, desconto = :desconto, valor_ipi = :valor_ipi, valor_nota = :valor_nota, pessoa = '$pessoa', frequencia = '$frequencia',compensacao = '$compensacao',centro_de_custo = '$centro_de_custo',sub_centro = '$sub_centro',reembolso = '$reembolso', classificacao = '$classificacao', usuario_lanc = '$id_usuario', arquivo = '$foto', tag = '$tag', pago = 'Não'");
}else{
...
$query->bindValue(":valor_bruto", "$valor_bruto");
$query->bindValue(":valor_seguro", "$valor_seguro");
$query->bindValue(":pis_pasep", "$pis_pasep");
$query->bindValue(":cofins", "$cofins");
$query->bindValue(":csll", "$csll");
$query->bindValue(":irrf", "$irrf");
$query->bindValue(":iss", "$iss");
...
if ($tipo_pagamento == 'Fatura - Viagem'){
	$tabela2 = 'despesas_viagens';

	if($id == ""){
		$query2 = $pdo->prepare("INSERT INTO $tabela2 SET id_pagamento = '$ult_id', descricao_viagem = '$descricao_pgto', fornecedor = '$fornecedor',num_fatura = '$numero_documento',valor_fatura = :valor, data_emiss_fat = '$data_emiss', data_venc_fat = '$data_venc',data_pgto_fat = '$data_pgto', usuario_lanc = '$id_usuario', arquivo = '$foto'");
	}
	else{
		$query2 = $pdo->prepare("UPDATE $tabela2 SET  descricao_viagem = '$descricao_pgto', fornecedor = '$fornecedor',num_fatura = '$numero_documento',valor_fatura = :valor, data_emiss_fat = '$data_emiss', data_venc_fat = '$data_venc', data_pgto_fat = '$data_pgto', usuario_lanc = '$id_usuario', arquivo = '$foto' where id_pagamento = '$id'");
	}
