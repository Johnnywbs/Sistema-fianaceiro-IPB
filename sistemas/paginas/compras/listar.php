...
if($vencidas != ""){
	$query = $pdo->query("SELECT * FROM $tabela where data_venc < curDate() and pago != 'Sim' and empresa = '$id_empresa' and tipo = 'Compra' order by data_venc asc");
}else{
	$query = $pdo->query("SELECT * FROM $tabela where data_venc >= '$data_inicial' and data_venc <= '$data_final' and pago LIKE '%$status%' and empresa = '$id_empresa' and tipo = 'Compra' order by id desc");
}
