...
if($vencidas != ""){
	$query = $pdo->query("SELECT * FROM $tabela where data_venc < curDate() and pago != 'Sim' and empresa = '$id_empresa' and tipo = 'Venda' order by data_venc asc");
}else{
	$query = $pdo->query("SELECT * FROM $tabela where data_lanc >= '$data_inicial' and data_lanc <= '$data_final' and pago LIKE '%$status%' and empresa = '$id_empresa' and tipo = 'Venda' order by id desc");
}
...
<td><i class="fa fa-square {$classe_pago} mr-1"></i> {$nome_pessoa}</td> 
		...
		<p style="color:#000">Confirmar Baixa da Conta? <a href="#" onclick="baixar('{$id}')"><span class="text-verde">Sim</span></a></p>
					...
					<big><a href="../rel_sistema/comprovante.php?id={$id}&imp=Não"  title="Ver Comprovante" target="_blank"><i class="fa fa-file-pdf-o" style="color:green"></i></a></big>
...
<br>
<div align="right">
<span style="margin-right: 25px">Vendas Vencidas: <span class="text-danger">R$ {$vencidasF}</span></span> 
<span style="margin-right: 25px">Vendas Pendentes: <span class="text-danger">R$ {$pendentesF}</span></span> 
<span style="margin-right: 25px">Vendas Pagas: <span class="text-verde">R$ {$recebidasF}</span></span> 
</div>
