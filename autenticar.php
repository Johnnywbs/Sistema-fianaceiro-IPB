...
if($total_reg > 0){
	...
	if($total_reg > 1){
		...
		$query = $pdo->prepare("SELECT * FROM usuarios WHERE (email = :usu or cpf = :usu) and senha_crip = :senha and ativo = 'Sim' ");
		$query->bindValue(":usu", "$usuario");
		$query->bindValue(":senha", "$senha_crip");
		$query->execute();
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		$total_reg2 = @count($res);

		if($total_reg2 == 0){
			echo '<script>alert("Usuários Inativos, contate o Administrador!")</script>';
